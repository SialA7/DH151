...
/* data.forEach(function(item,index){
	...
	// add data to sidebar using jquery
	$('.sidebar').append('<div class="sidebar-item">' + item.title + '</div>')
    //The above is one option, but the preferred option is to use a "Template literal"
    $('.sidebar').append(`<div class = "sidebar-item">${item.title}</div>`)

    // add data to sidebar with onclick interactive popup
    $('.sidebar').append(`<div class="sidebar-item" onclick="alert('you clicked ${item.title}')">${item.title}</div>`)

    // add data to sidebar with onclick flyTo event
	$('.sidebar').append(`<div class="sidebar-item" onclick="flyToIndex(${index})">${item.title}</div>`)
})
*/


// Now let's do it again with featureGroups!
